...
const scenarios = [
  {
    ...
    setup: "A striker is on a yellow card. Celebrating, he pulls up his shirt to reveal a plain white undershirt with 'RIP Mum' written on it in marker pen. His teammates hold back to let him have the moment. His mother died yesterday. The stadium falls quiet. Everyone in the ground knows the story.",
  ...
  },
  {
    ...
    setup: "During the penalty shootout in the World Cup final, a star striker misses his penalty. That was his last kick before retiring. As he walks back to his teammates, visibly distressed, you hear him admonish himself with a terrible racial slur. You are the only one who heard it.",
    question: "You are the only witness. What do you do?",
    correctRuling: "You must still act. Law 12 does not require a witness. Offensive or abusive language is a sending-off offence regardless of who heard it. The fact that only you heard it makes this harder to manage, not easier to ignore. Red card, and document exactly what you heard in your match report.",
    keyPrinciple: "Offensive or abusive language is a red card offence under Law 12. The law does not require a witness — only the referee's certainty. Being the only one who heard it makes it harder, not optional.",
  ...
  },
  {
    ...
    setup: "Near half-time, you award a penalty for a foul in the box. Players are already walking off. The player you just awarded the penalty comes over and says quietly: 'I've got to be honest, ref. I was well outside the box.' Your assistant didn't flag. You thought it was in. The kick hasn't been taken.",
    question: "The beneficiary is telling you it shouldn't stand. What do you do?",
    correctRuling: "The penalty stands. A player's word alone is not grounds to reverse a decision, even if that player is the one who benefited. Consult your assistant — if they saw nothing, the penalty stands. Law 5 permits changes only on the referee's own realisation or on advice from an assistant or fourth official.",
    keyPrinciple: "A player's word — even an honest confession from the beneficiary — cannot reverse a decision. Only the referee's own realisation or assistant advice can do that. Law 5.",
  ...
  },
  {
    ...
    setup: "A section of the away support has been chanting homophobic slurs throughout the game. You have already requested a PA warning. An openly gay home player scores, sprints to the corner and kisses his rainbow captain's armband. Missiles immediately land on the pitch. The fourth official radios that police are moving in.",
  ...
  },
  {
    ...
    minute: "12'",
    stakes: "Youth cup",
    setup: "A young player scores in a youth cup tie. You think it might have been handball, but on talking to the player you realise he has a prosthetic arm. The team sheet declared no disabilities. His coach steps forward with documentation: the player was bullied out of his previous club due to his limb-difference. His parents chose not to declare it to avoid more discrimination.",
...
const bonusScenarios = [
  {
    ...
    setup: "A penalty is awarded in the 88th minute. The kick is taken, the keeper saves it. VAR buzzes. The keeper moved off his line early. Then VAR buzzes again. Your earpiece crackles: the taker's feet were both off the ground when he struck the ball.",
  ...
  },
  {
    ...
    minute: "93'",
    stakes: "Relegation decider",
    setup: "Just before a corner, you notice the goalkeeper has a smartwatch on her wrist, partially hidden under tape. You call her over. She says her doctor has instructed her to wear it at all times to monitor her heart rate. She shows a medical bracelet on her other wrist as proof.",
    question: "What do you do next?",
  ...
  },
  {
    ...
    setup: "Play pauses for a minor injury. From the touchline, the home manager, who is deaf, uses sign language to communicate with his centre-back. You happen to be fluent in sign language. The instruction is unambiguous: injure the opposition's centre-forward, but make it look like an accident.",
    question: "What do you do next?",
  ...
  return (
    <div style={{
      ...
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        <span>{label}</span>
        <span style={{ fontSize: "14px", color: "#bbb", lineHeight: 1, display: "inline-block", transform: open ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}>▾</span>
      </button>
      {open && (
        <div style={{
          padding: "0 14px 13px", paddingTop: "12px",
...
export default function RefForADay() {
  ...
  const loadSaved = () => {
    try {
      const saved = JSON.parse(localStorage.getItem("referoo_state") || "{}");
      return saved;
    } catch { return {}; }
  };
  ...
  const [phase, setPhase] = useState(saved.phase && saved.phase !== "result" ? saved.phase : (saved.scenarioIndex > 0 ? "scenario" : "intro"));
  const [email, setEmail] = useState("");
  const [name, setName] = useState("");
  const [emailSubmitted, setEmailSubmitted] = useState(saved.emailSubmitted || false);
  const [emailLoading, setEmailLoading] = useState(false);
  const [npsScore, setNpsScore] = useState(saved.npsScore || null);
  const [bonusUnlocked, setBonusUnlocked] = useState(saved.bonusUnlocked || false);
  const [allScenarios, setAllScenarios] = useState(saved.bonusUnlocked ? [...scenarios, ...bonusScenarios] : scenarios);
  const [submitError, setSubmitError] = useState(null);

  // Persist state on key changes
  useEffect(() => {
    try {
      localStorage.setItem("referoo_state", JSON.stringify({
        scenarioIndex,
        phase: phase === "result" ? "scenario" : phase,
        bonusUnlocked,
        emailSubmitted,
        npsScore,
      }));
    } catch {}
  }, [scenarioIndex, phase, bonusUnlocked, emailSubmitted, npsScore]);
...
For correct_ruling: one sentence only. The bare decision. No explanation.

For referee_writeup: 2-3 sentences of practical refereeing insight. DO NOT restate the correct ruling — assume the reader has just read it. Go straight to the why, the consequences, or what the law is trying to achieve. Direct, conversational, address the reader as "you". No flowery language.
...

{
  ...
  "correct_ruling": "<one sentence, the bare decision only>",
  "referee_writeup": "<2-3 sentences, no restatement of the ruling, straight to the insight>"
}`;
      ...
      fetch("/api/response", {
        ...
        body: JSON.stringify({
          ...
          total_score: parsed.the_call.score + parsed.the_law.score + parsed.spirit_of_game.score,
          verdict: parsed.verdict,
        }),
      })
        .then(r => r.json().then(d => console.log("Response save:", r.status, d)))
        .catch(e => console.error("Response save failed:", e));
    } catch (err) {
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        html, body { background: #e0e0e0 !important; }
        ...
        .ruling-box {
          border: 2px solid #538d4e; border-radius: 4px;
          padding: 14px 16px; margin-bottom: 10px; background: #f0f7ef;
        }
        .ruling-label {
          font-family: 'Outfit', sans-serif;
          font-size: 13px; font-weight: 800; letter-spacing: 0.5px;
          color: #2d6a29; text-transform: uppercase; margin-bottom: 6px;
        }
        .ruling-text {
          font-family: 'Lora', serif;
          font-size: 14px; line-height: 1.65; color: #1a3318;
        }
        ...
        .law-banner-label {
          ...
          color: #7a5f00; text-transform: uppercase; margin-bottom: 6px;
        }
        .law-banner-text {
          font-family: 'Lora', serif;
          font-size: 14px; line-height: 1.65;
          font-weight: 600; color: #2a1f00;
        }
      ...
      <div className="app">
        ...
        {phase === "result" && scores && (
          <div className="results-section">
            {/* Episode label on results */}
            <div className="meta-row">
              <span className="episode-chip">{scenario.episode}</span>
              <span className="chip">{scenario.minute}</span>
              <span className="chip">{scenario.stakes}</span>
            </div>
            <div className="scenario-title" style={{ marginBottom: "12px" }}>{scenario.title}</div>

            {/* Collapsible recap */}
            <Accordion label="The scenario">{scenario.setup}</Accordion>
            <Accordion label="Your answer">{userResponse}</Accordion>

            {/* Your Score label */}
            <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: "11px", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "#888", marginBottom: "8px", marginTop: "8px" }}>Your Score</div>
            ...
            {/* Law banner */}
            <div style={{ marginTop: "16px" }}>
            <div className="law-banner fade-in" style={{ opacity: revealed ? 1 : 0, transitionDelay: "0.9s" }}>
              <div className="law-banner-label">📖 The law</div>
              <p className="law-banner-text">{scenario.relevantLaw}</p>
            </div>
            </div>
